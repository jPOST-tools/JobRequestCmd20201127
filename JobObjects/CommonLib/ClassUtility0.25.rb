...
class Hash
	def deep_merge_extension(target)
	result = {}
	...
	if target.kind_of?(Hash)
		target.each do |key, value|
			if value.kind_of?(Hash) then
				...
				result[key] = result[key].deep_merge_extension(value)
			else
...
#p fileAry,myJobObjKey,myJobTreeKey
#exit(0)
		if(myJobTreeKey.to_i().to_s() == myJobTreeKey)
			myJobTreeKey = myJobTreeKey.to_i()
		end
		inTypeAry.each{|fileType|
			if(fileAry[fileType].nil? == true)
				MyTools.printfAndExit(1,"fileAry[#{fileType}] is null.",caller(0))
			end
#p fileAry[fileType],myJobTreeKey
#exit(0)
...
#exit(0)
	...
	def MyTools.getInParam(aJobName,aArgv,aInFileKey,aOutFileKey,aTmpFileKey)
		...
		paramFile = aArgv[3]
#p myJobObjKey,myJobGrp,myJobTreeKey,paramFile
#exit(0)
...
#exit(0)
	...
	def MyTools.chekcHashKey(aHashData,aAry)
		aAry.each{|aKey|
	...
	def MyTools.filesCopy(aFileAry)
		aFileAry.each{|aFrom,aTo|
			if(File.exists?(aFrom) == false)
				next
			end
			if(File.exists?(aTo) == true)
				next
			end
			FileUtils.cp(aFrom,aTo)
		}
	end

	def MyTools.filesMove(aFileAry)
		aFileAry.each{|aFrom,aTo|
			if(File.exists?(aFrom) == false)
				next
			end
			FileUtils.mv(aFrom,aTo,{:force => true})
		}
	end

	def MyTools.execCmd(aCmdStr)
		rtc = system(aCmdStr)
		if(rtc != true)
			p rtc
			MyTools.printfAndExit(1,"system(#{aCmdStr}) is false.(#{$?})",caller(0))
		end
	end

	def MyTools.fileLock(aFileName)
		retryCount = 10
		while(retryCount != 0) do
			begin
				fp = File.open(aFileName,File::RDWR|File::CREAT|File::BINARY)
	...
	def MyTools.fileUnLock(aFp)
		aFp.flock(File::LOCK_UN)
		aFp.close()
	end
...
class CsvFileToArray
	...
	def exec(aSep="\t",aEncoding="UTF-8")
		...
		CSV.foreach(@inFile,{:col_sep => aSep, :encoding => aEncoding}){|aRaw|
			lineNo += 1
			if(lineNo == 1)
...
=begin
//------------------------------------------------------------------
	Class CsvFileToArray_DelTitleLine

	CSVファイルの内容をタイトル文字列をキーにした
	Arrayに変換する。
	中にタイトル文字と同じものがあった場合は、その行を
	削除する。
//------------------------------------------------------------------
=end
...
class CsvFileToArray_DelTitleLine
	...
	def creUseTitle(aRaw,aUseTitle)
		...
		aRaw.each_index{|i|
			...
			rtc[i] = aRaw[i]
			if(rtcTitleNoTop == nil)
				rtcTitleNoTop = i
			end
		}
		return [rtc,rtcTitleNoTop]
	end
...
#p aRaw
			if(lineNo == 1)
				(titleWk,checkTitleNo) = self.creUseTitle(aRaw,@useTitle)
#p titleWk,checkTitleNo
#exit(1)
				titleWk.each{|aVal|
					if(aVal.nil? == true)
						next
			...
			if(aRaw[checkTitleNo] == titleWk[checkTitleNo])
#p lineNo,titleWk[checkTitleNo]
#exit(0)
				next
			end
